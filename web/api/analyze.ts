...
- Se a lista trouxer MENOS de 10 comparáveis (abaixo do mínimo exigido), gere amostras apenas para os que houver (pode ser de 0 a 9 — NUNCA complete com amostras inventadas) e defina "dadosInsuficientes" como true. Em "dadosInsuficientesMotivo", explique objetivamente: quantos comparáveis reais foram encontrados num raio de até 1000m do imóvel avaliando e por que isso é insuficiente para uma precificação confiável pelo método comparativo direto. Reflita essa limitação também em "parecer.fundamentacao" e em "descricaoLaudo", sendo honesto sobre a baixa confiabilidade do valor estimado nesse cenário.
  ...
  amostrasProntas
    ? amostrasProntas.length > 0
      ? `\nAMOSTRAS JÁ HOMOGENEIZADAS (${amostrasProntas.length} — geradas numa passada anterior dedicada, NÃO gere o campo "amostras" nesta resposta, use só como contexto):\n${amostrasProntas
          ...
          .join('\n')}`
      : '\nAMOSTRAS JÁ HOMOGENEIZADAS: nenhuma (nenhum comparável real foi encontrado num raio de até 1000m). "dadosInsuficientes" deve ser true.'
    : comparaveisReais.length > 0
      ? `\nCOMPARÁVEIS REAIS ENCONTRADOS NA INTERNET (${comparaveisReais.length} encontrados, já geocodificados e com link verificado — esta é a ÚNICA fonte válida de amostras, NUNCA invente outras; use o endereço, a URL, a distância e o PREÇO EXATAMENTE como fornecidos):\n${comparaveisReais
          ...
          .join('\n')}`
      : '\nCOMPARÁVEIS REAIS ENCONTRADOS NA INTERNET: nenhum encontrado num raio de até 1000m. NÃO invente amostras — "amostras" deve ser [] e "dadosInsuficientes" deve ser true.'
}
