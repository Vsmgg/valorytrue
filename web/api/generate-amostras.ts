...
${comparaveisReais
  .map(
    (c, i) =>
      `${i + 1}. ${c.endereco}${c.areaM2 ? ` (${c.areaM2} m²)` : ''}${c.distanciaM !== null ? `, a ${c.distanciaM}m do imóvel avaliado` : ''} — Preço real do anúncio: R$ ${c.valorAnunciado} — URL: ${c.url}`,
  )
  .join('\n')}
